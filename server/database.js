...
export const databaseFunction = {
    ...
    insertRating: async (username, appuntoId, rating) => {
        ...
        const sql = `
            ...
            FROM appunto AS a
                     ...
                     JOIN categoriaAppunto AS ca ON a.id = ca.idAppunto
                     JOIN categoria AS cat ON ca.idCategoria = cat.id
            WHERE c.tipo IN (?)
              AND visibilita = 1
    ...
    updatePassword: async (username, oldPassword, newPassword) => {
        ...
        const [checkResults] = await db
            .promise()
            .query(checkSql, [username, oldPassword]);
    ...
    saveNewNote: async (note) => {
        ...
        const [getResults] = await db
            .promise()
            .query(getSql, [note.title, note.author]);
        if (getResults.length === 0) {
            ...
            try {
                ...
                const [results] = await db
                    .promise()
                    .query(sql, [note.title, userId[0].id, note.date]);
                return results;
            } catch (error) {
                return null;
            }
    ...
    saveComponent: async (element, id, position) => {
        ...
        try {
            ...
            const result = db
                .promise()
                .query(sql, [id, element.type, position, elemData]);
            return result;
        } catch (error) {
            console.log(error);
            return null;
    ...
    followUser: async (followerUsername, followedUsername) => {
        ...
        try {
            const [followerResults] = await db
                .promise()
                .query(getFollowerIdSql, [followerUsername]);
            const [followedResults] = await db
                .promise()
                .query(getFollowedIdSql, [followedUsername]);
            ...
            if (followerResults.length > 0 && followedResults.length > 0) {
                ...
                const [insertResults] = await db
                    .promise()
                    .query(insertFollowSql, [followerId, followedId]);
                return insertResults;
            } else {
                return null;
            }
        ...
        try {
            ...
            if (followerResults.length > 0 && followedResults.length > 0) {
                ...
                const [deleteResults] = await db
                    .promise()
                    .query(deleteFollowSql, [followerId, followedId]);
    ...
    followCategory: async (username, categoryName) => {
        ...
        try {
            ...
            const [categoryResults] = await db
                .promise()
                .query(getCategoryIdSql, [categoryName]);
            ...
            if (userResults.length > 0 && categoryResults.length > 0) {
                ...
                const [insertResults] = await db
                    .promise()
                    .query(insertFollowSql, [userId, categoryId]);
                return insertResults;
            } else {
                return null;
            }
    ...
    unfollowCategory: async (username, categoryName) => {
        ...
        try {
            ...
            if (userResults.length > 0 && categoryResults.length > 0) {
                ...
                const [deleteResults] = await db
                    .promise()
                    .query(deleteFollowSql, [userId, categoryId]);
                return deleteResults;
            } else {
                return null;
            }
    ...
    getFeedByCategories: async (categories) => {
    ...
    getFollowedCategories: async (username) => {
        ...
        const [categoriesNamesResult] = await db
            .promise()
            .query(getCategoriesNamesSql, [followedCategoriesIds]);
    ...
    removeCategoriesFromNote: async (noteTitle, author, categories) => {
        ...
        const [categoryIdsResults] = await db
            .promise()
            .query(getCategoryIdsSql, [categories]);
    ...
    searchUsers: async (searchString, currentUsername) => {
        ...
        try {
            const [results] = await db
                .promise()
                .query(sql, [searchString + "*", currentUsername]);
            return results;
        } catch (error) {
    ...
    isUserFollowedByUser: async (followerUsername, followedUsername) => {
        ...
        const [results] = await db
            .promise()
            .query(sql, [followerUsername, followedUsername]);
        return results[0].count > 0;
    },
